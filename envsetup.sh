...
function check_product()
{
    ...
    if (echo -n $1 | grep -q -e "^venom_") ; then
        VENOM_BUILD=$(echo -n $1 | sed -e 's/^venom_//g')
        export BUILD_NUMBER=$( (date +%s%N ; echo $VENOM_BUILD; hostname) | openssl sha1 | sed -e 's/.*=//g; s/ //g' | cut -c1-10 )
    else
        VENOM_BUILD=
    fi
    export VENOM_BUILD
...
function lunch()
{
    ...
    if [ $? -ne 0 ]
    then
        # if we can't find a product, try to grab it off the VenomProject-Devices GitHub
        T=$(gettop)
        cd $T > /dev/null
        vendor/venom/build/tools/roomservice.py $product
        cd - > /dev/null
        check_product $product
    else
        T=$(gettop)
        cd $T > /dev/null
        vendor/venom/build/tools/roomservice.py $product true
        cd - > /dev/null
    fi
